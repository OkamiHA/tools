...
func main() {
	...
	r.POST("/convert", func(c *gin.Context) {
		...
		if fileType == "doc" {
			err = convertPDFToDocx(pdfPath, docxPath)
			if err != nil {
...
func convertPDFToDocx(inputPDF, outputDocx string) error {
	cmd := exec.Command("libreoffice", "--headless", "--convert-to", "doc", "--outdir", filepath.Dir(outputDocx), inputPDF)
